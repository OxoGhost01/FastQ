...
fastq_pool_t *fastq_pool_create(const char *host, int port, int size)
{
    ...
    for (int i = 0; i < size; i++) {
        ...
        if (!p->conns[i] || p->conns[i]->err) {
            fastq_log(FASTQ_LOG_ERROR, "pool: failed to create conn %d: %s",
                      i, p->conns[i] ? p->conns[i]->errstr : "alloc error");
            fastq_pool_destroy(p);
            return NULL;
        }
    }

    fastq_log(FASTQ_LOG_INFO, "pool: created %d connections to %s:%d",
              size, host, port);
    return p;
}
